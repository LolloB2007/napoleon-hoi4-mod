#!/usr/bin/env python3
"""Compile editable content/*.py sources into checked-in HOI4 engine files."""
...
def compile_sources(root: Path) -> dict[str, bytes]:
    ...
    for source in sorted((root / 'content').glob('build_*.py')):
        ...
        for name, value in module.build(root).items():
            path = Path(name)
            if path.is_absolute() or '..' in path.parts or not (path.parts[0] in ALLOWED or name in DOCS):
                raise ValueError(f'unsafe generated path: {name}')
            if name in outputs:
                raise ValueError(f'duplicate generated output: {name}')
            outputs[name] = value.encode('utf-8') if isinstance(value, str) else value
    manifest = {name: hashlib.sha256(data).hexdigest() for name, data in sorted(outputs.items())}
    outputs['docs/generated_manifest.json'] = (json.dumps(manifest, indent=2) + '\n').encode()
    return outputs
