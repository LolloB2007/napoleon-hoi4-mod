...
The presentation pipeline gives every current focus a unique GFX registration, every current scripted event a custom period-styled picture, and every inline political/military leader an original portrait card. To keep the expanded 4,000+ focus repository practical, high-volume deep branches reuse a curated bank of 24 procedural engraved focus textures while retaining unique focus GFX IDs. It also supplies regime flags, three loading screens, 1789 bookmark art, UI ornaments, event stingers, historical division/corps naming groups, sailing-warship names and additional flavour events.
...
The event sound layer remains functional. A11 supplies the approved original/public-domain soundtrack layer.

A12 makes English-only localisation a build invariant: maintained release localisation lives only under localisation/english and uses the l_english header.
"""
    return out
...
def _patch_focuses(text,updates,interface):
    ...
    for start,end in reversed(spans):
        ...
        deep=("_deep_" in fid.lower() or "_depth_" in fid.lower())
        if deep:
            variant=_seed(fid)%24
