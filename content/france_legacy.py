...
Keep inherited dates and territorial outcomes. Guard execution as well as event
scheduling; do not change the owner's chronology/alternate-peace policy.
"""
...
RULES = {
 ...
 'french_revolution.12':'nap_fra_route_republican = yes has_country_flag = french_republic NOT = { has_country_flag = directory_active }',
    ...
    for filename in ('01_french_revolution.txt','02_napoleonic_wars.txt','03_collapse.txt'):
        ...
        for event in entries:
            ...
            if identifier=='french_revolution.8':
                ...
                event.value += parse('trigger = { nap_fra_route_republican = yes has_country_flag = french_republic NOT = { has_country_flag = directory_active } NOT = { has_country_flag = nap_legacy_french_revolution_12_settled } date > 1795.10.26 } mean_time_to_happen = { days = 1 }')
        ...
        output['events/'+filename] = '# Generated from retained source; preserves historical policy while guarding route and repeat execution.\n'+dumps(entries)
    return output
